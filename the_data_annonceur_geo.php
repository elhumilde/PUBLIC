...
LEFT JOIN lien_web AS wb ON wb.`code_firme` = firmes.`code_firme` AND wb.num_ordre =1 
...
if ($row["code_firme"]){
$rub = $myPDO->query("SELECT lien.`code_rubrique`as code_rubrique ,Lib_Rubrique FROM `lien_rubrique_internet` lien INNER JOIN rubriques r on r.`code_rubrique`=lien.`Code_Rubrique` WHERE lien.`code_firme` LIKE '".$row["code_firme"]."' AND lien.`editable` LIKE '+' LIMIT 1"); 
...
WHERE  concat("MA",a.code_firme) ="'.$row["code_firme"].'"  and now() <= fin_mise_en_ligne  and interr=0 and a.format<>"" ');
...
while ($row_production = $sth->fetch())
{
    ...
    if ($row_production['code_produit'] == 'ML')
    {
        ...
        if ($test_ml == 0)
        {
                                                                                     ...
                                                                                     ${"villes$i"}[]=  array( 'name' => $row_production["ville"]  );
    ...
    $myString_ville = $row_production['ville_supp'];
            ...
            $rubriques[] = array(
                ...
                'slug' => slugify(adilsoft_string_latin($row_production["rubrique"])) ,
                                                             ...
                                                             $renvois = $myPDO->query("SELECT Lib_Rubrique, Code_Rubrique FROM `rubrique_voir` INNER JOIN `rubriques` ON `Code_Rubrique` = `code_rubrique1`WHERE `code_rubrique2` LIKE'".$row_production["code_rubrique"]."'"); 

                                                               if (!empty($renvois)) { 
                                                                         
                                                                         foreach ($renvois as $ren ) {
                                                                                      ${"mot_cles$i"}[]=       array('mot' =>  $ren["Lib_Rubrique"], 'code' => $ren["Code_Rubrique"], 'slug' => '', );
                                                                                    }
                ...
                'slug' => '',
                                                             ...
                                                             $renvois = $myPDO->query("SELECT Lib_Rubrique, Code_Rubrique FROM `rubrique_voir` INNER JOIN `rubriques` ON `Code_Rubrique` = `code_rubrique1`WHERE `code_rubrique2` LIKE'".$row_production["code_rubrique"]."'"); 
                                                               ...
                                                               if (!empty($renvois)) { 
                                                                         ...
                                                                         foreach ($renvois as $ren ) {
                                                                                      $mot_cles1[]=       array('mot' =>  $ren["Lib_Rubrique"], 'code' => $ren["Code_Rubrique"], 'slug' => '', );
                                                                                    }
...
$module_logo_arr =  array('module_logo' => htmlspecialchars_decode($module_logo));
        ...
        "Content-Type: application/json" 
         ...
         $conn = curl_init();
         $requestURL = 'https://elastic:xYOqNi8bikEe09cm6vDVELBs@edicom-go.es.eastus2.azure.elastic-cloud.com/_bulk';
